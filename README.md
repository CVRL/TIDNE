...
Note that video generation with our model is not currently implemented. 

**Disclaimer:** A main finding in our work is that the dataset used to train our model contains significantly fewer subjects than FFHQ (52 < 70k) with many more iris images per subject, ultimately leading to "identity leakage." As such, training images tend to show up as synthetic samples. In order to maintain data integrity and preserve privacy, we advise users to proceed with caution. Sample diversity ("faker" images) can be throttled via the truncation value at the cost of image quality.

Outputs from the above commands are placed under `out/*.png`, controlled by `--outdir`. Downloaded network pickles (from NVIDIA) are cached under `$HOME/.cache/dnnlib`, which can be overridden by setting the `DNNLIB_CACHE_DIR` environment variable. The default PyTorch extension build directory is `$HOME/.cache/torch_extensions`, which can be overridden by setting `TORCH_EXTENSIONS_DIR`.
